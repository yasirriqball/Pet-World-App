...
export default function VetsReviewsPage() {
  ...
  const [openVetId, setOpenVetId] = useState<string | null>(null)
        ...
        {Object.values(vets).map((vet) => {
          ...
          const avgRating =
            reviewsArr.length > 0
              ...
              : null
          const isOpen = openVetId === vet.userId
          return (
            <Card key={vet.userId} className="overflow-hidden relative">
              <CardHeader className="flex flex-col items-start gap-2 pb-2">
                ...
                {avgRating && (
                  <div className="flex items-center gap-2 mt-2 bg-yellow-50 dark:bg-yellow-900/30 px-3 py-1 rounded-lg shadow-sm">
                    ...
                    <span className="text-sm font-medium text-muted-foreground">
                      / 5 ({reviewsArr.length} review{reviewsArr.length !== 1 ? "s" : ""})
                    </span>
                  </div>
                )}
                <button
                  className="mt-3 px-4 py-1 rounded-md bg-primary text-primary-foreground font-medium shadow hover:bg-primary/90 transition"
                  onClick={() => setOpenVetId(isOpen ? null : vet.userId)}
                >
                  {isOpen ? "Hide Reviews" : `See Reviews${reviewsArr.length > 0 ? ` (${reviewsArr.length})` : ""}`}
                </button>
              </CardHeader>
              {isOpen && (
                <CardContent>
                  {reviewsArr.length === 0 ? (
                    ...
                    <div className="space-y-6 transition-all duration-300">
                      {reviewsArr
                        ...
                        .map((review, idx) => (
                          <div key={idx} className="bg-muted/50 dark:bg-muted/30 rounded-xl p-4 shadow-sm flex flex-col gap-2 border border-muted/30">
                            <div className="flex items-center gap-3">
                              {review.userImage ? (
                                <img
                                ...
                                <div className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center text-lg text-gray-500 font-bold">
                            ...
                            <div className="text-base text-gray-800 dark:text-gray-200 pl-1 mt-1">
                              {review.comment}
                            </div>
                          </div>
                        ))}
